...
import static abr.auto.R.id.irCenter;
import static abr.auto.R.id.irLeft;
import static abr.auto.R.id.irRight;
...
public class Main_activity extends Activity implements IOIOLooperProvider, SensorEventListener        // implements IOIOLooperProvider: from IOIOActivity
{
	...
	public void onCreate(Bundle savedInstanceState) 
	{
		...
		irLeftText = (TextView) findViewById(irLeft);
		irCenterText = (TextView) findViewById(irCenter);
		irRightText = (TextView) findViewById(irRight);
	...
	public final void onSensorChanged(SensorEvent event) {
		float irCenter = m_ioio_thread.irCenterReading;
		float irLeft = m_ioio_thread.irLeftReading;
		float irRight = m_ioio_thread.irRightReading;
		if (m_ioio_thread != null) {
			...
			if (btnStartStop.isChecked()) {
				...
				if (irCenter > 0.5) {
					m_ioio_thread.move(0.5f, 0.5f, false, false);
					m_ioio_thread.move(0.5f, 0.5f, true, false);
				}
				if (irLeft > 0.5) {
					m_ioio_thread.move(0.5f, 0.25f, true, false);
				}
				if (irRight > 0.5) {
					m_ioio_thread.move(0.25f, 0.5f, false, true);
				}
